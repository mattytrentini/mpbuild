...
def docker_build_cmd(
    ...
    if variant is not None:
        ...
        if v is None:
            raise ValueError(f"Variant '{variant}' not found for board '{board.name}': Valid variants are: {[v.name for v in board.variants]}")
...
def build_board(
    ...
    This is the command is used by the command line version and
    therefore
